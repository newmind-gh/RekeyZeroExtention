...
The model selector contains the available browser-local WebLLM models plus **Gemini · API** and **DeepSeek · API**. Selecting a local model downloads and loads it in the browser. Gemini and DeepSeek are configured under **Gemini / DeepSeek API settings** in the Side Panel. The user enters the model name and their own API key, and the Extension calls the selected provider directly without a RekeyZero backend proxy. Saved setups use the existing Mapping Profile storage with the `ai_fill_setup` kind and the guarded transfer executor. **Fill** runs the selected setup; **Reset** clears only the active batch and retains saved setups.

API keys are never read from a repository `.env`. By default a key is kept only in extension session storage. The user can explicitly select **Remember this key on this device** to keep it in extension-local browser storage. RekeyZero restricts those storage areas to trusted extension contexts, but it does not encrypt remembered keys; anyone who can access the browser profile may be able to recover them. Keys are excluded from normal exports and diagnostics, and removing a provider deletes its stored key and provider host permission.
